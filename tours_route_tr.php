...
$sqlFile = fopen("tours_route_tr_insert.sql", "w") or die("Unable to open file!");
                        ...
                        foreach($translations as $translation) {
                            ...
                            if ($resultLanguage->num_rows > 0) {
                                while($rowLanguage = $resultLanguage->fetch_assoc()) {
                                    ...
                                    // Included, Excluded, and Route lines fields
                                    $fields = ['route_lines' => 28];

                                    foreach ($fields as $field => $tmplvarid) {
                                        $sqlField = "SELECT value FROM modx_site_tmplvar_contentvalues_full WHERE tmplvarid = $tmplvarid AND contentid = $translateId";
                                        $resultField = $conn->query($sqlField);

                                        if ($resultField->num_rows > 0) {
                                            $value = $resultField->fetch_assoc()["value"];

                                            if ($field === 'route_lines') { // If the field is route_lines, no need for formatting
