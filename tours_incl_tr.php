...
$sqlFile = fopen("tours_inc_tr_insert.sql", "w") or die("Unable to open file!");
                        ...
                        foreach($translations as $translation) {
                            ...
                            if ($resultLanguage->num_rows > 0) {
                                while($rowLanguage = $resultLanguage->fetch_assoc()) {
                                    ...
                                    $includedExcludedFields = ['included' => 32, 'excluded' => 36];

                                    foreach ($includedExcludedFields as $field => $tmplvarid) {
                                        $sqlIncludedExcluded = "SELECT value FROM modx_site_tmplvar_contentvalues_full WHERE tmplvarid = $tmplvarid AND contentid = $translateId";
                                        $resultIncludedExcluded = $conn->query($sqlIncludedExcluded);

                                        if ($resultIncludedExcluded->num_rows > 0) {
                                            $value = $resultIncludedExcluded->fetch_assoc()["value"];

                                            // Formatting values
                                            $value = preg_replace('/<li[^>]*>/', '', $value, 1);
